...
template <typename T>
void set_stereo_params(cv::Ptr<T> stereo,
                       int minDisparity,
                       int numDisparities,
                       int blockSize,
                       int P1,
                       ...
                       int speckleRange)
{
    ...
    if constexpr (std::is_same_v<T, cv::StereoSGBM> || std::is_same_v<T, cv::stereo::StereoBinarySGBM>)
    {
        stereo->setP1(P1);
        stereo->setP2(P2);
    }
...
template <typename T>
auto make_right_matcher(cv::Ptr<T> left)
{
    if constexpr (std::is_same_v<T, cv::stereo::StereoBinarySGBM>)
    {
        int min_disp = left->getMinDisparity();
        int num_disp = left->getNumDisparities();
        int wsize = left->getBlockSize();
        auto right = T::create(-(min_disp + num_disp) + 1, num_disp, wsize);
        right->setUniquenessRatio(0);
        right->setP1(left->getP1());
        right->setP2(left->getP2());
...
template <typename T>
auto make_wls_filter(cv::Ptr<T> left)
{
    if constexpr (std::is_same_v<T, cv::stereo::StereoBinarySGBM>)
    {
        auto left_fake = cv::StereoSGBM::create();
        set_stereo_params(left_fake,
                          left->getMinDisparity(),
                          left->getNumDisparities(),
                          left->getBlockSize(),
                          left->getP1(),
                          left->getP2(),
                          left->getDisp12MaxDiff(),
                          left->getPreFilterCap(),
    ...
    {
        return cv::ximgproc::createDisparityWLSFilter(left);
    }
}
...
void DepthCamera::step()
{
    ...
    if (!latest_left->stale || !latest_right->stale)
        return;
    tick();
...
bool DepthCamera::configure_matchers()
{
    ...
    stereo_left = StereoMatcherType::create(minDisparity, numDisparities, blockSize);
    set_stereo_params(stereo_left,
                      ...
                      speckleWindowSize,
                      speckleRange);
    stereo_right = make_right_matcher(stereo_left);
    filter = make_wls_filter(stereo_left);
